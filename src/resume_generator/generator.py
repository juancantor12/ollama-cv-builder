...
class Generator:
	...
	def write_profile(self, profile: dict) -> None:
		"""Does."""
		self.docx_factory.add_text(text=profile["name"], bold=True, uppercase=True, font_size=18, centered=True)
	...
	def write_education(self, educations: dict) -> None:
		"""Does."""
		self.docx_factory.add_text(text='EDUCATION', bold=True, uppercase=True, font_size=13)
		...
		for education in educations:
			...
			for bullet in education.get("bullet_list", []):
				self.docx_factory.add_text(text=self.docx_factory.bulleted(bullet), font_size=10)

			self.docx_factory.add_line_breaks(1)

	def write_experience(self, experiences: dict) -> None:
		"""Does."""
		self.docx_factory.add_text(text="EXPERIENCE", bold=True, uppercase=True, font_size=13)
		self.docx_factory.add_horizontal_line()
		for experience in experiences:
			...
			if "skills" in experience:
				self.docx_factory.add_text(text=f"Skills: {' • '.join(experience['skills'])}", italic=True)
	...
	def write_others(self, data: dict, sections: list) -> None:
		...
		for section in sections:
			self.docx_factory.add_text(text=section, bold=True, uppercase=True, font_size=13)
			self.docx_factory.add_horizontal_line()
	...
	def generate(self, data: dict) -> str:
		"""Generates the docx file based on the given data."""
		self.write_profile(data['profile_info'])
		self.write_education(data['education'])
		self.write_experience(data['experience'])
		self.write_others(data, ["continuous_learning", "personal_projects", "personal_development"])
